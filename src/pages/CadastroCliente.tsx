...
import { Loader2, ArrowLeft, Users, Eye, EyeOff, CheckCircle2, XCircle } from "lucide-react";
...
function validateFullName(name: string): string | null {
  ...
  if (parts.length < 2) return "Informe pelo menos nome e sobrenome (ex: Maria Silva)";
...
// ── Formata WhatsApp em tempo real ─────────────────────────────────────────
    ...
    <div className="mt-2 space-y-2">
      ...
      <div className="grid grid-cols-1 gap-1">
        {PASSWORD_RULES.map(rule => (
          <div key={rule.label} className={`flex items-center gap-1.5 text-xs transition-all ${rule.test(password) ? "text-accent" : "text-muted-foreground"}`}>
            {rule.test(password)
...
const CadastroCliente = () => {
  ...
  const validate = (): boolean => {
    ...
    const nameErr = validateFullName(fields.full_name);
    ...
    if (fields.password !== fields.confirm_password)
      errs.confirm_password = "As senhas não coincidem";

    if (!aceitouTermos)
      errs.termos = "Você precisa aceitar os Termos de Uso";
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    toast.success("Bem-vindo ao Editaí! 🎉");
        ...
        <div className="bg-gradient-card border border-border/50 rounded-2xl p-8 shadow-elegant">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Crie sua conta gratuitamente</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4" noValidate>
            ...
            <div>
              ...
              <Input
                id="full_name" placeholder="Maria Silva" autoComplete="name"
                value={fields.full_name} onChange={e => update("full_name", e.target.value)}
            ...
            <div>
              ...
              <Input
                id="phone" type="tel" placeholder="(11) 99999-9999" autoComplete="tel"
                value={fields.phone}
              ...
              {errors.phone
                ...
                : <p className="text-xs text-muted-foreground mt-1">Para comunicação sobre seus pedidos</p>
              }
            </div>
            ...
            <Button type="submit" variant="hero" className="w-full" disabled={loading}>
              {loading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Criar conta de cliente"}
            </Button>
          </form>
