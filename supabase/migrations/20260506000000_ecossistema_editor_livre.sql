-- ═══════════════════════════════════════════════════════════════
-- EDITAÍ — Sistema de Ecossistema: Editor Livre + Score Bilateral
-- ═══════════════════════════════════════════════════════════════

-- ── 1. Adicionar campos no editor_profiles ───────────────────
...
-- Editores já aprovados são considerados verificados
UPDATE public.editor_profiles SET is_verified = true WHERE status = 'aprovado';

-- ── 2. Score de cliente no profiles ──────────────────────────
...
-- ── 3. Avaliação bilateral de clientes por editores ───────────
...
-- ── 4. Controle de revisões por pedido ───────────────────────
...
-- ── 6. Trigger: atualizar score do cliente após avaliação ─────
...
BEGIN
  ...
  SET
    client_score = (
      SELECT ROUND(
        (AVG(rating) * 0.5 + AVG(brief_clarity) * 0.3 + AVG(communication) * 0.2)::NUMERIC,
        2
      )
...
-- ── 7. Trigger: travar brief ao aceitar pedido ────────────────
...
BEGIN
  IF NEW.status = 'aceito' AND OLD.status = 'aguardando_aceite' THEN
    ...
    -- Define max_revisions baseado no pacote
